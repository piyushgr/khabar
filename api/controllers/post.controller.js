...
export const updatePost = async (req, res, next) => {
  try {
    ...
    if (!(req.user.isAdmin === 2 || req.params.userId === req.user.id)) {
      return next(
    ...
    next(error); // Pass any caught errors to the global error handler
  }
};
...
export const getSinglePost = async (req, res, next) => {
  if (!req.user.isAdmin == 2 || req.params.userId !== req.user.id) {
    return next(errorHandler(403, "You are not allowed to get this post"));
  }
  try {
